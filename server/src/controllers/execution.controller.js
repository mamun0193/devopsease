...
export const executeDeployment = asyncHandler(async (req, res) => {
        ...
        if (!revision) {
            throw new NotFoundError('Artifact revision not found');
        }
        
        if (revision.approvalStatus !== 'APPROVED') {
            throw new ValidationError('Cannot execute unapproved revision');
        }
        ...
        res.status(202).json(standardResponse(execution));
...
export const getExecution = asyncHandler(async (req, res) => {
        ...
        res.status(200).json(standardResponse(execution));
});
...
export const getExecutionLogs = asyncHandler(async (req, res) => {
        ...
        if (!execution || !execution.logPath) {
            throw new NotFoundError('Logs not found');
        }
        
        const logs = await readLogFile(execution.logPath);
        res.status(200).json(standardResponse({ logs }));
});

export const rollbackExecution = asyncHandler(async (req, res) => {
        const { id } = req.params;
