...
import { ValidationError, UnauthorizedError, ConflictError, RateLimitError } from "../utils/AppError.js";
...
export const register = asyncHandler(async (req, res) => {
    ...
    res.status(201).json(standardResponse({ message: "Account created successfully" }));
});
...
export const login = asyncHandler(async (req, res) => {
    ...
    if (!bruteCheck.allowed) {
        ...
        res.set('Retry-After', bruteCheck.retryAfter);
...
export const refresh = asyncHandler(async (req, res) => {
    ...
    res.json(standardResponse({ expiresAt: Date.now() + ACCESS_TOKEN_MAX_AGE }));
});
...
export const logout = asyncHandler(async (req, res) => {
    ...
    res.json(standardResponse({ loggedOut: true }));
});
